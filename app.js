...
const selectedHymnContainer = document.querySelector('.selectedHymnContainer')
const suggestionContainer = document.querySelector('.suggestionContainer')
const searchNavBar = document.querySelector('.searchBar')
const searchCenter = document.querySelector('.search-center')
const hymnIndex = document.querySelector('.hymnindex')
const HymnNum = document.querySelector('.HymnNum')
const submitBtn = document.querySelector('#submit-btn')
const hymnIndexNav = document.querySelector('.HymnIndexNav')
const hymnNumIdx = document.querySelector('.hymnNumIdx')
...
text.addEventListener('keydown', e => {
   ...
   // filter hymns for matching titles
   const matchingHymns = Hymns.filter((hymn) => {
      const hymnTitle = hymn.hymnTitle.toLocaleLowerCase();
      return hymnTitle.includes(searchTerm)

   })

   // display suggestion
   matchingHymns.forEach((hymn) => {
      const suggestionElement = document.createElement('div')
      suggestionElement.textContent = hymn.hymnTitle
      suggestionElement.className = 'suggestionElement'
      suggestionElement.classList.add('suggestion')

      //  Add a click event listener to select the hymn
      suggestionElement.addEventListener('click', () => {
         // Render the selected hymn title and lyrics
         selectedHymnContainer.innerHTML = `
         <h2 class='site'>${hymn.hymnTitle}</h2>
         <ul class='vex'>
         ${hymn.hymnLyrics.map((verseObj) => `<p>${verseObj.verse}</p>`).join('')}
         </ul>
         `
         // clear the search input and suggestions
         text.value = ''
         suggestionContainer.innerHTML = ''
         HymnNum.textContent = hymn.id
         container.style.display = 'none'
         nav_Center.classList.add('remove')
         Navcenter.classList.add('remove')
         toggle.classList.add('show')
         banner.style.display = 'none'
         selectedHymnContainer.style.display = 'block'
...
searchNavBar.addEventListener('click', () => {
   ...
   selectedHymnContainer.style.display = 'none'
...
const hymnTitle = document.querySelector('.hymnTitlesContainer')
const idxHymnContainer = document.querySelector('.indexHymnContainer')
const hymnTitCont = document.querySelector('.hymnTitCont')
const idxhymncont = document.querySelector('.idxhymncont')

// Function to render all hymns title
function renderHymnTitles() {
   hymnTitle.innerHTML = '';
   Hymns.forEach((hymn) => {
      const titleElement = document.createElement('li')
      titleElement.className = 'hymnTitle'
      titleElement.textContent = hymn.hymnTitle;
      titleElement.classList.add('hymn-title')
      // Add click event to render hymn lyrics when clicked
      titleElement.addEventListener('click', () => {
         renderHymnLyrics(hymn)

         hymnNumIdx.innerHTML = hymn.id

         idxhymncont.style.display = 'block'
         hymnTitCont.style.display = 'none'
         indexHymnContainerNav.style.display = 'block'
         toggle.classList.add('show')
         hymnIndexNav.style.display = 'none'
      })
      hymnTitle.appendChild(titleElement);
   })
}

// Function to render hymn lyrics
function renderHymnLyrics(hymn) {
   idxHymnContainer.innerHTML = '';
   const titleElement = document.createElement('h2');
   titleElement.className = 'idxElement'
   // console.log(titleElement);
   titleElement.textContent = hymn.hymnTitle
   const lyricsElement = document.createElement('ul')
   lyricsElement.className = 'lyricElement'
   hymn.hymnLyrics.forEach((verseObj) => {
      const verseElement = document.createElement('p');
      verseElement.textContent = verseObj.verse
      lyricsElement.appendChild(verseElement)
   })
   idxHymnContainer.appendChild(titleElement)
   idxHymnContainer.appendChild(lyricsElement)
}



// Add click event to the "hymnIndex" button
hymnIndex.addEventListener('click', () => {
   renderHymnTitles()
   idxHymnContainer.innerHTML = ''
   // clear the selected hymn container
   container.style.display = 'none'
   hymnTitCont.style.display = 'block'
   Navcenter.classList.add('remove')
   hymnIndexNav.style.display = 'block'
   search.style.display = 'none'
   zoom.classList.add('show')

})

hymnIndexNav.addEventListener('click', () => {
   container.style.display = 'block'
   search.style.display = 'flex'
   zoom.classList.remove('show')
   hymnTitCont.style.display = 'none'
...
const indexHymnContainerNav = document.querySelector('.indexHymnContainerNav')

indexHymnContainerNav.addEventListener('click', () => {
   indexHymnContainerNav.style.display = 'none'
   hymnIndexNav.style.display = 'block'
   idxhymncont.style.display = 'none'
   hymnTitCont.style.display = 'block'
   toggle.classList.remove('show')
...
slider.addEventListener('click', ((e) => {
   ...
   selectedHymnContainer.classList.toggle('dark-mode')
   idxhymncont.classList.toggle('dark-mode')
}))
